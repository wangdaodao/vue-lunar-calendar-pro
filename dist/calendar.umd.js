...
/* harmony default export */ var componentsvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    render: function render(year, month) {
      ...
      var beforeRender = this.beforeRender; // 重新绑定数据->渲染
    ...
    monthDetail: function monthDetail(year, month, weekCount) {
      // 计算用的month为传入month - 1
      var monthToUse = month - 1; // 构建该月第一天date对象

      var firstDate = new Date(year, monthToUse); // 获取周几信息,0表示周日
    ...
    turn: function turn(step) {
      ...
      date.setMonth(date.getMonth() + step); // 获取要渲染的年份、月份

      var toRenderYear = date.getFullYear();
      var toRenderMonth = date.getMonth() + 1; // 渲染
